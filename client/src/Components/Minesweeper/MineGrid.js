...
function MineGrid() {
	const GRID_SIZE = 16;
	const BOMB_COUNT = 40;
	const [squareList, setSquareList] = useState(getInitGrid(GRID_SIZE, BOMB_COUNT));
	...
	useEffect(() => {
		const flagsRemaining = BOMB_COUNT - squareList.filter((square) => square.flagged).length;
	...
	function flipAdjacentSquares(x, y, list) {
		...
		if (currentSquare.bomb) return;
			...
			if (currentSquare.adjacentBombs === 0) {
				...
				adjacentSquaresWithoutBombsNotVisited.forEach((square) => flipAdjacentSquares(square.x, square.y, list));
			}
		}
	}
	...
	function handleLeftClick(x, y) {
		if (!gameOver) {
			...
			if (clickedSquare.bomb) {
				...
				if (GRID_SIZE * GRID_SIZE - squareList.filter((square) => square.flipped).length === BOMB_COUNT) endGame(true);
			}
